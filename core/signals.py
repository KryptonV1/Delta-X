...
EV_RESET       = "reset"
EV_NEAR_ENTRY  = "near_entry"    # kept for main.py import compat (unused)
...
class BBMATracker:
    ...
    def _build_signal(
        ...
        # ── SL (per SOP: "di luar low Extreme" + "perlu ruang bernafas") ──
        # Use ref_candle low but enforce minimum 1% below entry so SL is
        ...
        # ── TP levels (per SOP: TP1 < TP2 < TP3 ascending) ──────────────
        # TP1 = nearer of bb_middle or ma5_high (above entry)
        # TP2 = bb_upper
        # If TP2 < TP1 (BB squeezed), swap so order is always ascending.
        tp1_candidates = [
            x for x in [float(last["bb_middle"]), float(last["ma5_high"])]
            if x > entry
        ]
        raw_tp1 = min(tp1_candidates) if tp1_candidates else float(last["bb_middle"])
        raw_tp2 = float(last["bb_upper"])

        # Enforce ascending: TP1 < TP2
        if raw_tp2 <= raw_tp1:
            tp1, tp2 = raw_tp2, raw_tp1
        else:
            tp1, tp2 = raw_tp1, raw_tp2

        tp3 = tp2 + abs(tp1 - sl)               # projection beyond TP2
